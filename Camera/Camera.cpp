...
Camera::Camera()
{
	...
	mProjectionMatrix = XMMatrixPerspectiveFovLH(3.14159265f / 2.0f, 4.0f / 3.0f, 0.1f, 1000.0f);
...
{
	XMMATRIX mat =  XMMatrixLookAtRH(mPosition, mLookAt + mPosition, mUp);
	return mat;
}
...
XMMATRIX Camera::GetOrthographicProjectionMatrix(float viewWidth, float viewHeight) const
{
